...
class Entity (object):
    ...
    Note that many of the methods defined here take a `aliases`
    ...
    _sql_tables = {}
    """
    A dictionary with three keys: `primary`, `tables` and
    `managed`. The `primary` key should map to a list of primary key
    ...
    def _sql_fields(cls):
        """
        Just like `nfldb.Entity._sql_columns`, except derived fields
        are also returned.
        """
        return cls._sql_columns() + cls._sql_tables['derived']

    @classmethod
    def _sql_select_from(cls, fields=None, aliases=None):
        """
        ...
        columns = cls._sql_select_fields(fields, aliases=aliases)
        extra_tables = ''
        ...
        '''.format(columns=', '.join(columns),
                   from_table=from_table, as_from_table=as_from_table,
    ...
    def _sql_select_fields(cls, fields, wrap=None, aliases=None):
        ...
        return ['%s AS %s' % (sql(f), f) for f in fields]
    ...
    def _sql_join_all(cls_from, cls_tos):
        ...
        while len(tos) > 0:
            # dists :: [(f, t, distance)]
            dists = [(f, t, f._sql_relation_distance(t))
                     for f in froms for t in tos]
            dists = filter(lambda (f, t, d): d is not None, dists)
            assert len(dists) > 0, \
                'Cannot compute distances between sets. From: %s, To: %s' \
                % (froms, tos)
            more_general = filter(lambda (f, t, d): d < 0, dists)
            more_specific = filter(lambda (f, t, d): d > 0, dists)
            print('dists', dists)
            ...
            if more_general:
                f, t, _ = max(more_general, key=lambda (f, t, d): d)
                print('least_general', f, t)
                joins += add_join(f, t)
            if more_specific:
                f, t, _ = min(more_specific, key=lambda (f, t, d): d)
                print('least_specific', f, t)
                joins += add_join(f, t)
            print('-' * 79)
        return joins
    ...
    def _sql_join_to_all(cls_from, cls_to, from_table=None,
        ...
        fields in `cls_to._sql_fields()` are available for filtering.
    ...
    def _save(self, cursor):
        ...
        for table, table_fields in self._sql_tables['tables']:
            if table in self._sql_tables['managed']:
                vals = _as_row(prim + table_fields, self)
                _upsert(cursor, table, vals, vals[0:len(prim)])
